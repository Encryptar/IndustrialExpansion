...
public class CompositeFurnaceMenu extends AbstractContainerMenu {
    private final CompositeFurnaceEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public CompositeFurnaceMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(11));
    }

    public CompositeFurnaceMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuInit.COMPOSITE_FURNACE_MENU.get(), pContainerId);
        checkContainerSize(inv, 10);
        blockEntity = ((CompositeFurnaceEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            this.addSlot(new SlotItemHandler(handler, 6, 84, 35));
            this.addSlot(new SlotItemHandler(handler, 7, 48, 53));
            this.addSlot(new SlotItemHandler(handler, 8, 66, 53));
            this.addSlot(new SlotItemHandler(handler, 9, 84, 53));
            this.addSlot(new ModResultSlot(handler, 10, 141, 35));
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, BlockInit.COMPOSITE_FURNACE.get());
    }
